 ...
 *  STATIC PROTOTYPES
 **********************/
static void xpt7603_corr(int16_t * x, int16_t * y);
static void xpt7603_avg(int16_t * x, int16_t * y);
...
/**
 * Initialize the XPT7603
 */
void xpt7603_init(void)
{
 ...
 */
bool xpt7603_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
...
static void xpt7603_corr(int16_t * x, int16_t * y)
{
    ...
    *y = swap_tmp;
    ...
    if((*x) > XPT7603_X_MIN)(*x) -= XPT7603_X_MIN;
    else(*x) = 0;

    if((*y) > XPT7603_Y_MIN)(*y) -= XPT7603_Y_MIN;
    else(*y) = 0;

    (*x) = (uint32_t)((uint32_t)(*x) * XPT7603_HOR_RES) /
           (XPT7603_X_MAX - XPT7603_X_MIN);

    (*y) = (uint32_t)((uint32_t)(*y) * XPT7603_VER_RES) /
           (XPT7603_Y_MAX - XPT7603_Y_MIN);
...
static void xpt7603_avg(int16_t * x, int16_t * y)
{
    ...
    avg_buf_x[0] = *x;
    avg_buf_y[0] = *y;
    if(avg_last < XPT7603_AVG) avg_last++;
